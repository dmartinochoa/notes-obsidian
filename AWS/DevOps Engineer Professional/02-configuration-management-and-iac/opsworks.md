...
- OpsWorks autoscaling is not as flexible as ASG. Instances have to be pre-created and they will be started based on time schedule or load
- **Apps** represent code stored in a repository that we want to install on application server instances
- **Deployments**: 
    - OpsWorks will run a deploy or undeploy command on a given number of instances
...
- CloudWatch Events: we can create rules for reacting to OpsWorks instance changes
