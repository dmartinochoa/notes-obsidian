...
- Used to execute documents (scripts) or just to run simple commands
...
- Run Command can be invoked with EventBridge
...
- We get notifications with Amazon EventBridge
...
- Requires an **Instance Identity Role**: a type of IAM role with no permissions beyond identifying the EC2 instance to AWS Services such as Systems Manager
...
- We have to install SSM Agent on Greengrass Core devices, they will register themselves as managed nodes in SSM
...
- We must add permissions to the Token Exchange Role to communicate with SSM
...
- Compliance can be sent to Security Hub
...
- The goal of OpsCenter is to reduce the mean time to resolve issues
