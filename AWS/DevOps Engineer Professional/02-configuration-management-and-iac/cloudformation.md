...
- Templates have to be uploaded to S3 and then be referenced in CloudFormation
...
- Parameters can be controlled by all of these settings:
    - Types:
        - `String`
        - `Number`
        - `CommaDelimitedList`
...
- We can not delete a CloudFormation stack if its outputs are being referenced by another stack
- Output `Export` block: has to be specified in order for the output to be able to be imported in another template
...
- `Fn::Sub`:
    - Used to substitute values in strings
...
- In case a rollback failure happens we have to manually fix the issue and then use `ContinueUpdateRollback` API from Console
...
- `InsufficientCapabilitiesException`: this exception is returned if the capabilities were not allowed
...
- `DeletionPolicy=Retain`:
    - To be specified on resource to preserve/backup in case of CloudFormation deletes
...
- WaitConditions: tells CloudFormation to wait until it receives a signal from `cfn-signal`
    ...
    - We attach a `CreationPolicy`: how many signals we want to see and for how long we want to wait
...
- It is a way to define that a specific resource should be created after another one was already created

## CloudFormation StackSets

- Used to create/update/delete stacks across multiple accounts and regions, all in a single operation
...
- ChangeSets won't tell us in advance if the update will be successful
