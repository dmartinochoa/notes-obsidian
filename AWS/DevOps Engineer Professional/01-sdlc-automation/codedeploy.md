...
- CodeDeploy supports 2 kinds of deployments:
...
- CodeDeploy with EC2 - In-place Deployments:
    ...
    - Deployment hooks:
        - In case we use a Load Balancer: `BeforeBlockTraffic` -> `BlockTraffic` -> `AfterBlockTraffic` -> ... (see `appspec.yml` below)
- CodeDeploy with EC2 - Blue/Green Deployments:
    ...
    - Instance termination:
        - `BlueInstanceTerminationOption`: we can specify whether to terminate the original EC2 instance after a successful deployment
