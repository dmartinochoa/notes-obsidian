...
- Can contain `env` - environment variables (for more details, see environment variables below). The type of these can be:
...
- Example: [buildspec.yml](AWS/DevOps%20Engineer%20Professional/01-sdlc-automation/buildspec.yml)
...
- Commands:
    ...
    - Each phase can have a `finally` block: the commands from this block are executed whether or not the actual commands from the `commands` block succeeded
- Artifacts: 
...
- Cache:
    - These are intermediary files or dependencies used for build
...
- Service roles need an IAM permission to be able to log into ECR
...
- CodeBuild Service Role: allows CodeBuild to access AWS resources on our behalf
...
- To create a test report we have to add a `Report Group` in `buildspec.yml` file with information about tests