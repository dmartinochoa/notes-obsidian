...
- **Regional Edge Cache**: larger version of an edge location, but there are fewer of them. Provides another layer of caching. If we are using S3 origins, the region edge location is not used in case there is a cache miss for the edge location. Only custom origin can use the regional edge cache!
...
- Once a request is pattern matched against a behavior, it will become subject to the behavior's configurations which can be the following:
...
- An edge location views an object as not expired when it is within its TTL period
...
- Cache invalidations are performed in a distribution and they apply to all edge locations (it takes time)
...
- Process of adding alternate domain names:
    ...
    - The result is we need to add an SSL certificate whether we are using HTTPS or not
...
- Host header happens after that at Layer 7. It allows us to specify to which application we want to connect in case multiple applications run on the same server
- TLS encryption happens before deciding which application we want to access
- In 2003 an extension was added to TLS: SNI - allowing us to specify which domain we want to access
- Older browsers do not necessarily support SNI. CFN needs to allocate dedicated IP addresses for these users, at extra charge
- CFN can be used in SNI mode (free) or allocating extra IP addresses ($600 per month)
- For S3 origin, we don't need to apply certificates for the origin protocol. For ALB/EC2/on-prem we can have public certificates which need to match the DNS name of the origin

## Origin Types and Architecture

- Origins are the locations from where CF goes to get content
- If there is a cache miss in case of a request, then an origin fetch occurs
- Origin groups allow us to add resiliency. We can group origins together and have an origin group used by the behavior
...
- S3 origin configurations:
    ...
    - Origin Access Identity: allows us to give CF a virtual identity and use this to access the bucket
...
- Content retrieval techniques:
    ...
    - Cookies
    - Request Headers
- When using CFN all of this data reaches CloudFront first and then can be forwarded to the origin
...
- S3 origins:
    ...
    - Essentially the CloudFront distribution "becomes" the OAI, meaning that this identity can be used in S3 bucket policies
...
- CloudFront can run in 2 different modes:
    ...
    - Private: requests to CloudFront need to be made with a signed url or cookie
...
- There are 2 types of restriction:
    ...
    - 3rd Party Geolocation:
        ...
        - Requires an application server in front of CloudFront, which controls whether the customer has access to the content or not
