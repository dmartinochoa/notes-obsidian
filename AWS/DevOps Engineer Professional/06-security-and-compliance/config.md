...
- Problems AWS Config Solves:
    ...
    - Find out how an ALB configuration changed over time
...
- Custom rules:
    ...
    - Trigger types for custom rules are same as for managed rules:
        ...
        - Periodic
    - Scope of the trigger: we can define which resource the rule applies to, example database instance, EC2 instance, etc. We can use tags as well instead of resource types
...
- We can create custom configuration recorders to record only the resources we specify
...
- If we use AWS Organizations, we don't need individual authorization of the accounts
