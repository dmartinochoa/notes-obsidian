...
- It is a global service allowing to manage multiple AWS accounts from an organization
...
- It is automatically added to all new member accounts created with AWS Organizations
- Must be created manually if we invite an existing account

## Multi Account Strategies

- Create accounts per department, per cost center, per dev/test/prod, based on regulatory restrictions (using SCP), for better isolation, to have separate per-account service limits, isolated accounts for logging, etc.
