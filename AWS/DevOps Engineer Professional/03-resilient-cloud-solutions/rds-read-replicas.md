...
- Exception for managed services, since RDS read replicas are considered to be managed services, within the same region we don't pay a fee for inter-AZ data transfer
...
- In case we go for a database from single-AZ setup to multi-AZ:
    ...
    - Only thing to do is click on `modify` for the database
...
![Aurora Auto Scaling](aurora-auto-scaling.png)
