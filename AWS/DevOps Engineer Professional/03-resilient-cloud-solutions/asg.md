...
- Both allow us to specify the AMI, the instance type, a key pair, security groups, and the other parameters that we use to launch EC2 instances (tags, user-data, etc.)
...
- Launch Templates:
    ...
    - We can provision both On-Demand and Spot instances (or a mix of the two)
...
- Warm up period: number of seconds ASG has to wait until the metric of a new instance can be taken into consideration for further ASG action

## ALB Integration

- Slow start duration: in the target group we can set a duration period during which the number of requests will be gradually increased to the new instance
...
- Scale in protection: this can be set on a specific instance. If a scale in action happens, instances protected by scale in protection won't be terminated
...
![ASG Lifecycle Hooks](lifecycle-hooks.png)
...
- Default termination policy: detects which AZ has the most instances with at least one instance which does not have termination protection. Within an AZ the default termination policy behavior is the following:
    ...
    3. If there are multiple instances with the latest launch configuration, terminate the one which is the closest to the next billing hour. If there are multiple of these, terminate one at random
...
- Warm pool size settings:
    - Warm pool size: calculated as the difference between the ASG group's maximum desired capacity and its desired capacity
...
![ASG Lifecycle Hooks with Warm Pools](lifecycle-hooks-warm-pools.png)
...
- In place (one LB, one target group, one ASG): instance state is mutated
