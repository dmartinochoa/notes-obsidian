...
- It is an alternative to ECS, having a similar goal but different API
- EKS supports EC2 launch mode if we want to deploy worker nodes or Fargate to deploy serverless containers
- Use case for EKS: if our company already is using Kubernetes on-premises or on another cloud and wants to migrate to AWS
...
- Self-Managed Nodes:
    ...
    - We can then use the prebuilt AMI - Amazon EKS Optimized AMI; or we can build our own AMI
