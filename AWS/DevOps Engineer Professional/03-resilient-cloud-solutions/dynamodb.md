...
- Millions of records per second, trillions of rows, 100TB of storage
...
- Low cost
- Provides **Standard** and **Infrequent Access** (IA) table classes
...
| Strong requirements about how the data should be modeled | All the data should be present in one row                     |
| Ability to do joins, aggregations, and computations         | Do not support joins, can't perform aggregations such as "SUM"  |
...
- Partition keys are hashed in order to know which partition the data will go to
...
- Batching allows us to save latency by reducing the number of API calls
...
- **Scan**
    ...
    - For faster performance we can use parallel scan
        - Way more RCU!
    - Can use a combination of *ProjectionExpression + FilterExpression* (no charge to RCU)
...
- We must define an RCU/WCU for the index
- *GSI can be created and modified after original table creation!*
...
- It can be activated without doing any code change in the application which uses the database
...
- They can react to this changelog by:
    ...
    - etc.
- There are 2 kinds of stream processing offered for DynamoDB:
    - DynamoDB Streams:
        - 24 hours retention
        - Limited number of consumers
        - Can be processed with Lambda Triggers or DynamoDB Stream Kinesis Adapter
...
- We can choose what type of information ends up in the stream:
...
- Pattern:
    ...
    - Reader clients read the metadata which contains the object path and reads the object from the path
- Index objects in S3:
...
- Copy table:
    ...
    - Option 2: create a backup, restore it into a new table
    - Option 3: Scan + Write
...
- We can export a table to S3. To do this we must enable PITR:
    ...
    - When doing an export, the read capacity of the table won't be affected
    - Use cases:
