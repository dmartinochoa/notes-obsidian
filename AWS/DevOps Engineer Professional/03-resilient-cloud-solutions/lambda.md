...
- They are limited in time - they require short execution times
...
- Main ones:
    - API Gateway: allows us to create an external API which invokes the Lambda function
    - Application Load Balancer: similar to API gateway, API gateway may give many more API functionalities (example: authentication)
    - CloudWatch Events and EventBridge: we can react to events happening in the cloud. Provides a solution for having a scheduled Lambda function
    ...
    - Kinesis
    - S3 Events: async invocation, react to changes happening in S3 buckets
    - CloudFront
    - SNS and SQS
    - AWS Cognito
...
- When we work on a Lambda function, we work on a version named **$LATEST**
...
- We can define multiple aliases and have them point at different Lambda versions
- Aliases are mutable
...
- Concurrency limit per account per region: by default up to 1000 concurrent executions
- In case we need a higher amount of concurrent execution for our account, we can open a support ticket
...
- Limitations: each new Lambda execution will have a new connection to the EFS Access Point, we should watch out for connection limits and connection burst limits
