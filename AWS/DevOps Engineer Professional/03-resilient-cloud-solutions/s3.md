...
- If uploading more than 5 GB, we must use multi-part upload
- Each object in S3 can have metadata, tags, and (optional) version ID

## S3 Versioning

- Versioning should be enabled at the bucket level
- Versioning means: objects uploaded with the same key will increment the version
...
## S3 Monitoring Options Overview

```
Four main monitoring approaches:
...
```
Every HTTP request made to bucket:
→ Requester identity
→ Bucket name
→ Object key (specific file) ✅
→ Request time
→ HTTP method
→ HTTP status code
→ Bytes transferred
...
EventBridge can detect natively
No CloudTrail configuration needed
```

### Data Events (must enable, extra cost):

```
Object-level operations:
→ GetObject ← someone downloaded file
→ PutObject ← someone uploaded file
→ DeleteObject ← someone deleted file
→ CopyObject
→ HeadObject
→ ListObjects
→ RestoreObject
...
These are data plane operations
Must explicitly enable per bucket or all buckets
Additional cost per event recorded
```
### CloudTrail vs S3 Access Logs:

|                       | CloudTrail                   | S3 Access Logs     |
| --------------------- | ---------------------------- | ------------------ |
| **Identity**          | Full IAM identity ✅          | IP address only    |
| **Real-time**         | Near real-time               | Best effort, hours |
| **Cost**              | Extra for data events        | Storage only       |
| **EventBridge**       | ✅ Required for object events | ❌                  |
| **Format**            | JSON                         | Space-delimited    |
| **Management events** | ✅ Free                       | ❌                  |
| **Data events**       | ✅ (paid)                     | ✅ (all requests)   |
| **Cross-account**     | ✅ Organization trail         | ✅ Different bucket |
...
CloudTrail data events enabled
    ↓
S3 object operation occurs (GetObject, PutObject)
    ↓
...
EventBridge rule matches pattern
    ↓
Target triggered (Lambda, SNS, SQS, etc)

Critical: EventBridge needs CloudTrail
for S3 object-level events ← exam fact

Management events (bucket level):
→ EventBridge receives natively
...
## 3. CloudWatch Metrics for S3

### Bucket-level metrics (free, daily):
...
### Request metrics (paid, must enable):

```
Must enable per bucket or prefix:
→ Extra cost per metric
...
HeadRequests → HEAD operations
PostRequests → POST operations
ListRequests → LIST operations
BytesDownloaded → bytes sent to requesters
BytesUploaded → bytes received from requesters
...
### Metric filter levels:

```
Entire bucket:
→ All requests aggregated
→ Cannot see per-object

Prefix level:
→ All requests to prefix aggregated
→ e.g. logs/ folder

Object level:
→ Must pre-configure metric filter
→ Only for pre-configured objects
→ Expensive and unscalable for many objects
→ Cannot dynamically track all objects ← exam fact

Limitation:
→ Cannot answer "which file downloaded most"
→ Use S3 access logs for that
```

### CloudWatch Alarms on S3:

```
Common alarms:
→ 4xxErrors > threshold → alert
→ 5xxErrors > threshold → alert
→ BucketSizeBytes > threshold → alert
→ BytesDownloaded > threshold → cost alert

Cannot alarm on:
→ Specific object access
→ Per-file metrics (without pre-configuration)
```
...
Config tracks CONFIGURATION not ACCESS
Different from CloudTrail/access logs
```
...
→ Checks if logging enabled
→ Trigger: Configuration changes ✅ Periodic ✅

s3-bucket-versioning-enabled:
→ Checks if versioning enabled
→ Trigger: Configuration changes ✅

s3-bucket-public-read-prohibited:
→ Checks no public read access
→ Trigger: Configuration changes ✅ Periodic ✅

s3-bucket-public-write-prohibited:
→ Checks no public write access
→ Trigger: Configuration changes ✅ Periodic ✅
...
→ Checks bucket policy enforces HTTPS
→ Trigger: Configuration changes ✅

s3-bucket-cross-region-replication-enabled:
→ Checks CRR configured
→ Trigger: Configuration changes ✅

s3-bucket-default-lock-enabled:
→ Checks Object Lock enabled
→ Trigger: Configuration changes ✅

s3-lifecycle-policy-check:
→ Checks lifecycle policy configured
→ Trigger: Configuration changes ✅
```

### Config + Auto Remediation for S3:

```
s3-bucket-logging-enabled violated
    ↓
Config detects NON_COMPLIANT
    ↓
Auto remediation triggers
    ↓
SSM Automation: AWS-ConfigureS3BucketLogging ← pre-built ✅
    ↓
Logging enabled automatically

Other pre-built remediations:
...
```
Config records configuration history:
→ When was versioning enabled?
→ When did bucket policy change?
→ Who changed encryption settings?
→ What did bucket policy look like 30 days ago?
...
S3 Bucket
    │
    ├── Server Access Logs → S3 bucket → Athena
    │   (all requests, per-object, IP level)
    │
    ├── CloudTrail Data Events → CloudTrail
    │   (who accessed what, IAM identity)
    │   ↓
    │   EventBridge → Lambda/SNS/SQS
    │   (real-time reactions to object events)
    │
    ├── CloudWatch Metrics
    │   (aggregate counts, alarms)
    │   ↓
...
```
"Who downloaded specific file?"
→ CloudTrail data events (IAM identity)
→ OR S3 access logs (IP address)

"Which files downloaded most?"
→ S3 access logs + Athena

"React when file uploaded to S3"
→ S3 Event Notifications → Lambda
→ OR CloudTrail + EventBridge → Lambda

"Alert when bucket made public"
→ Config rule → EventBridge → SNS
→ OR CloudTrail management event → EventBridge → SNS

"Auto-fix non-compliant bucket settings"
→ Config + SSM Automation remediation

"Audit bucket configuration changes over time"
→ AWS Config configuration history

"Monitor request error rates"
→ CloudWatch request metrics + alarms

"Compliance report on all buckets"
→ Config aggregator across accounts

"Centralized audit trail across accounts"
...
```
S3 Access Logs:
→ Cannot send directly to CloudWatch ❌
→ Must go to S3 bucket first
→ Best effort delivery (not real-time)
→ Per-object visibility ✅
→ IP address only (not IAM identity)

CloudTrail:
→ Data events must be explicitly enabled
→ Extra cost for data events
→ Full IAM identity ✅
→ Required for EventBridge object-level events
→ Management events free and default

CloudWatch:
→ Cannot track per-object without pre-configuration
→ Request metrics must be enabled (cost)
→ Bucket metrics free but daily only

Config:
→ Tracks configuration not access
→ Cannot detect who accessed objects
→ Many pre-built S3 managed rules
→ Pre-built SSM remediation documents for S3
→ Configuration changes trigger = near real-time

EventBridge + S3:
→ Bucket-level events = no CloudTrail needed
→ Object-level events = CloudTrail required ← exam trap

Cross-account monitoring:
→ CloudTrail Organization trail = single trail all accounts
→ Config aggregator = compliance across accounts
→ S3 access logs = can send to different account bucket
```
...
- The request won't be fulfilled unless the other origin allows for the request, using CORS Headers (ex: `Access-Control-Allow-Origin`, `Access-Control-Allow-Method`)
...
- This applies to:
    - PUT of new objects
    - PUT (overwrite) of existing objects
    - DELETE operations
- As soon as an object is written or deleted, any subsequent read will return the latest version
- Strong consistency is provided at no additional cost and with no performance impact
- This eliminates the previous eventual consistency behavior for overwrites and deletes
...
- It is nice to get protected against unintentional file encryptions (example: ransomware)
- Deleting a file in S3 bucket just adds a delete marker on the versioning
- To delete a bucket, we need to remove all the files and their versions from it

## S3 MFA-Delete

- MFA forces users to generate a code on a device (usually a mobile phone) before doing important operations on S3
...
- Bucket policies are evaluated before the default encryption flag
...
- For delete operations:
    - If we delete without a version ID, S3 adds a delete marker (not replicated by default)
    - If we delete an object with version ID, it deletes the object from the source bucket, operation is not replicated
    - **Delete marker replication can be optionally enabled** in the replication rule settings
...
- We can restrict who can access our distribution by using:
    ...
    - Blacklist: prevent our users from accessing our content if they are in one of the countries on a blacklist of banned countries
...
- Used for audit and report on the replication and encryption status of our objects
- Use cases:
    - Business
    - Compliance
    - Regulatory needs
...
- Inventory setup: we have to set up a target bucket with a policy, inventory data will be saved here
...
- Each item in Glacier is called an *Archive*. An Archive can be up to 40TB in size
...
- For infrequently accessed objects, we can move them to standard IA
...
- Our application can achieve at least 3500 PUT/COPY/POST/DELETE and 5500 GET/HEAD requests per second per prefix in a bucket
...
- KMS quota increases can be requested through AWS Service Quotas
...
- S3 Transfer Acceleration:
    - Increase the transfer speed by transferring the file to an AWS edge location which will forward the data to the S3 bucket in the target region
...
- React to changes, events happening in a bucket. Example of events:
    ...
    - `S3:ObjectRestore`
    - `S3:Replication`
- Use cases: generate thumbnail if images are uploaded to S3
- Event consumers can be:
    - SNS
    - SQS
    - Lambda Functions
- S3 events notifications typically deliver events in seconds, sometimes can take over a minute
...
- Provides 3 types of operations:
    ...
    - Delete: single operation, can be achieved by using Glacier Rest API or by specifying the archive ID
...
- Vault lock policies are used for regulatory and compliance requirements:
    ...
    - Example: forbid deleting an archive if it is less than 1 year old
...
- If it takes more than a week to transfer over the network, it is probably a good idea to use Snowball

## Snowball Transferring Process

1. Request a Snowball from AWS console delivery
...
- It is a bridge between on-premises data and cloud data in S3
...
- **File Gateway**:
    ...
    - *Automating Cache Refresh*: 
        - A File Gateway feature that enables us to automatically refresh File Gateway cache to stay up to date with the changes in the S3 bucket
        - Ensures that users are not accessing stale data on their file shares
